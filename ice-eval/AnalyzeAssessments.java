...
public class AnalyzeAssessments {
	...
	public static void main (String[] args) 
		...
		if (args.length != 4) {
			System.err.println ("Review requires 4 arguments:");
			System.err.println ("     queries assessments slots relation");
			System.exit(0);
		}
...
if(!docAndOffsetList.contains("ENG")) continue;
		...
		PrintWriter triplesWriter = new PrintWriter(new FileWriter (relation + "-keyTriples"));
		for (String trip : tripleSet)
			triplesWriter.println(trip);
		triplesWriter.close();
                writeLists(ids, relation);
                System.out.println(ids.size() + " documents for relation " + relation);
	}
	...
	static Set<String> retrieveDocIds (String docAndOffsetList) {
        ...
        static void writeLists (Set<String> docIds, String relation) throws IOException {
            PrintWriter docListWriter = new PrintWriter (new FileWriter (relation + "-docList"));
            PrintWriter fileListWriter = new PrintWriter (new FileWriter (relation + "-fileList"));
		...
		String path = id2DocPath.get(docId);
                ...
                Path to   = FileSystems.getDefault().getPath(relation + "-docs", docId + ".sgm");
                Files.copy(from, to);
                docListWriter.println(docId);
