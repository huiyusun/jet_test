...
import edu.nyu.jet.scorer.ScoreAceTriplesCountPatterns;
...
public class Ace {
	...
	public static PathRelationExtractor pathRelationExtractor;

	/**
	 * if true, case information is not considered significant (for finding names or sentence boundaries).
	 */
	...
	/**
	 * if true, output confidence for events and event arguments as part of APF (non-standard APF).
	 */
	static boolean writeEventConfidence = false;

	/**
	 * generate ACE annotation files (in APF) format for a list of documents. Takes four to eight command line parameters:
	 * <BR>
	 ...
	 * outDirectory: the path of the directory in which APF files are to be written <BR>
	 * parseDir: the path of the directory containing parse trees [optional] <BR>
	 * glarfDir: the path of the directory containing GLARF tuples [optional] <BR>
	 * glarfSuffix: the file suffix for GLARF files <BR>
	 * parseSuffix: the file suffix for parse files <BR>
	 * For each <I>file</I> in <I>filelist</I>, the document is read from <I>docDirectory.file</I>.sgm and the APF file is
	 * written to <I>outDirectory.file</I>.sgm.apf
	 */
	...
	public static void main(String[] args) throws IOException {
		...
		BufferedWriter writer = new BufferedWriter(
				new FileWriter("/Users/nuist/documents/NlpResearch/ice-eval/results_subtypes_individual", true)); // append
		...
		for (int i = 0; i <= 0; i++) {
			...
			// adjust parameters
			pathRelationExtractor.setKThreshold(4);
			pathRelationExtractor.setK(1);
			pathRelationExtractor.setNegDiscount(1);

			pathRelationExtractor.loadAllRules(dir + "patterns_subtypes.pos", dir + "patterns_subtypes.neg", 100000);
			...
			// testing each individual rule with and without its subtype
			// String posPattern = (String) pathRelationExtractor.loadRules(dir + "patterns_subtypes.pos", limit,
			// "INDIVIDUAL-CUMULATIVE");
			// String negPattern = (String) pathRelationExtractor.loadNeg(dir + "patterns_subtypes.neg", limit, "INDIVIDUAL");

			// training and testing for dependency weights
			// pathRelationExtractor.loadRules(dir + "patterns.pos");
			// pathRelationExtractor.loadNeg(dir + "patterns.neg");
			// PathMatcher.loadDepWeights(dir + "weights_dep"); // all
			// parameterString = "train:v1 test:v1";
			...
			// parameterString = "pos:" + posCount + " neg:" + negCount;
			...
			while ((currentDocPath = reader.readLine()) != null) {
				...
				if (debug) {
					...
					try {
						...
						logger.error("Error processing document " + fileList + "(" + docCount + ")=" + currentDocPath, e);
					}
				}
			}
			...
			// ScoreAceTriplesCountPatterns.score(); // score triples and generate correct and incorrect count of each pattern
			...
			// for individual pattern with and without subtypes
			String improved = "NO";
			// String type = "S:";
			if (score > prevScore)
				improved = "YES";
			else if (score <= prevScore && score > prevScore - 0.0001)
				improved = "SAME";
			prevScore = score;
			// if ((limit % 2) == 0)
			// type = "T:";
			// parameterString = posPattern + " = " + improved;
			...
			System.out.println(parameterString + " = " + scoresStr);
			writer.write(parameterString + " = " + scoresStr + "\n");
			reader.close();
		}
		// }
		// }

		System.out.println("[BEST] " + bestParameterString + " = " + "F:" + bestScore);
		writer.write("[BEST] " + bestParameterString + " = " + "F:" + bestScore + "\n");
		writer.write("\n");
	...
	public static void initForFileIO(String[] args) throws IOException {
		...
		if (args.length < 4 || args.length > 8) {
			...
			logger.error(
					"    properties filelist documentDir outputDir" + " [parseDir [glarfDir [glarfSuffix [parseSuffix]]]]");
			System.exit(1);
		}
	...
	/**
	 * initialize ACE: read property file and load all resources specified by the properties file.
	 */
	...
	public static void init(String propertyFile) throws IOException {
		...
		if (relationDepPathPosFile != null && relationDepPathNegFile != null && embeddingFile != null) {
			loadPosAndNegModel(relationDepPathPosFile, relationDepPathNegFile, embeddingFile);
		}
		...
		EventTagger.EVENT_PROBABILITY_THRESHOLD = getConfigDouble("Ace.EventModels.eventProbabilityThreshold",
				EventTagger.EVENT_PROBABILITY_THRESHOLD);
		EventTagger.ARGUMENT_PROBABILITY_THRESHOLD = getConfigDouble("Ace.EventModels.argumentProbabilityThreshold",
	...
	/**
	 * if <CODE>paramName</CODE> is a numeric-valued parameter in the Jet configuration file, returns that value as a
	 * <CODE>double</CODE>, else returns <CODE>defaultValue</CODE>.
	 */
	...
	/**
	 * process a single document to generate an Ace APF file, catching any exceptions which occur.
	 */
	...
	public static void processFileAndCatchError(String currentDocPath) throws IOException {
		try {
			...
			logger.error("Error processing document " + docCount + " = " + currentDocPath, e);
		}
	}
	 ...
	 *          The full path of the file containing the source document
	 */
	...
	public static void processFile(String currentDocPath) throws IOException {
		...
		if (perfectMentions) {
			// doc = new ExternalDocument("sgml", docDir + "perfect parses/" +
			// currentDocPath);
			String textFile = docDir + currentDocPath;
			String keyFile = docDir + currentDocPathBase + ".apf.xml";
			// String keyFile = docDir + currentDocPathBase +
			// ".mentions.apf.xml"; //<< for corefeval
			// String keyFile = docDir + currentDocPathBase +
	 ...
	 *
	 * @param doc
	 *          The (Jet) source document
	 *
	 * @param sourceId
	 *          The source document id; typically the 'sourceFile', but without any file extension. If no DOCID is
	 *          specified within the document text) as the base for the names of all entities, relations, events, etc.
	 *
	 * @param sourceFile
	 *          The name of the source document. This is used to fill the source file field of the AceDocument. If 'doc'
	 *          is read from a file, should be the file name, including extension, but not including directory path.
	 *
	 * @param docPathBase
	 *          The full path to the source document, with the file extension removed; used to access corresponding parse
	 *          and GLARF files. Should be null if these files are not being used
	 *
	 * @return The AceDocument
	 */

	public static AceDocument processDocument(Document doc, String sourceId, String sourceFile, String docPathBase)
			throws IOException {
		doc.stretchAll();
		// process document
		...
		if (eve != null) {
			...
			DepPathRelationTagger.findRelations(docId, doc, aceDoc, pathRelationExtractor);
		}
	...
	/**
	 * returns the document ID of Document <CODE>doc</CODE>, if found, else returns <CODE>null</CODE>. It looks for <br>
	 ...
	 * the <B>DOCID</B> feature of a <B>document</B> annotation (for GALE ASR transcripts); or <br>
	...
	/**
	 * return true if either all the letters in span are lower case, or the fraction of letters which are upper case
	 * exceeds MAX_UPPER. Either condition is an indication that case is not significant in detecting names or sentence
	 * boundaries.
	 */
	...
	public static boolean allLowerCase(Document doc, Span span) {
		...
		return (countUpper == 0) || (countUpper > totalLetters * MAX_UPPER);
	}

	static String[] functionWord = { "a ", "an ", "the ", "his ", "her ", "its ", "against ", "as ", "at ", "by ",
			"due to ", "for ", "from ", "in ", "into ", "of ", "over ", "to ", "with ", "within ", "and ", "not ", "or ",
			"can ", "be ", "will " };

	/**
	 * returns true if Span <CODE>span</CODE> of Document <CODE>doc</CODE> appears to be capitalized as a title: if there
	 * are no words beginning with a lower-case letter except for a small list of function words (articles, possessive
	...
	public static boolean titleCase(Document doc, Span span) {
		...
		loop: for (int i = 0; i < text.length() - 1; i++) {
			if (Character.isWhitespace(text.charAt(i)) && Character.isLowerCase(text.charAt(i + 1))) {
	...
	/**
	 * returns a file path after removing the extension (period and following characters), if any, from the file name.
	 */
	...
	public static void buildAceEntities(Document doc, String docId, AceDocument aceDoc) {
		...
		if (entities != null) {
			for (int ientity = 0; ientity < entities.size(); ientity++) {
				AceEntity aceEntity = buildEntity(entities.get(ientity), ientity, doc, docId, docText);
				if (aceEntity != null)
	...
	private static AceEntity buildEntity(Annotation entity, int ientity, Document doc, String docId, String docText) {
		...
		// boolean generic = !AceDocument.ace2004 && isGeneric(doc,
		// firstMention);
		...
		aceEntityNo++;
		if (entityTrace)
			logger.trace("Generating ace entity " + aceEntityNo + " (internal entity " + ientity + ") = "
					+ Resolve.normalizeName(doc.text(firstMention)) + " [" + aceType + "]");
	...
	/**
	 * write the information for <CODE>mention</CODE> with head <CODE>head</CODE> in APF format.
	 */

	private static AceEntityMention buildMention(Annotation mention, Annotation head, String mentionID, String entityType,
	...
	/**
	 * determine the mention type of a mention (NOMINAL, PRONOUN, or NAME) from its <CODE>head</CODE>.
	 */
	...
	private static String mentionType(Annotation head, Annotation mention) {
		...
		// if (mention.get("preName-1") != null || mention.get("nameMod-1") !=
		// null ||
		// cat == "adj")
		// return "PRE";
		// else
		if (cat == "n" || cat == "title" || cat == "tv" || cat == "v")
			return "NOMINAL";
		else if (cat == "pro" || cat == "det" /* for possessives - his, its */ || cat == "adj" || cat == "ven" || cat == "q"
	...
	static final String[] genericFriendlyDeterminers = { "no", "neither", "any", "many", "every", "each" };
	static final String[] clearGenericPronouns = { "everyone", "anyone", "everybody", "anybody", "something", "who",
			"whoever", "whomever", "wherever", "whatever", "where" };
	...
	private static boolean isGeneric(Document doc, Annotation mention) {
		...
		if (headC.get("cat") == "n") {
			...
			if (vg != null) {
				...
				if (vpa != null && vpa.get("tense") != "past" && vpa.get("aspect") == null) {
			...
			return false;
		} else if (headC.get("cat") == "pro" || headC.get("cat") == "np" || headC.get("cat") == "det") {
			String pronoun = SynFun.getHead(doc, mention);
			return in(pronoun, clearGenericPronouns) || in(pronoun, genericFriendlyDeterminers); // <<
			// added
			// Oct.
			// 10
		} else
			/* head is a name */ return false;
	}
	...
	/**
	 * tag the document <CODE>doc</CODE> for time expressions, and create (and add to <CODE>aceDoc</CODE> an AceTimex
	 * object for each time expression.
	 */
