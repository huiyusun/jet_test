...
public class DepPath {
	...
	/**
	 * Constructor that allows the user to set spans for both arguments If arguments are set, the linearization process
	 * will avoid putting words in arguments on the linearized path.
	 *
	 * @param fromPosn
	 *          start position of the head of arg1
	 * @param toPosn
	 *          start poistion of the head of arg2
	 * @param arg1
	 *          Full span of the first argument
	 * @param arg2
	 *          Span of the second argument
	 */
	...
	/**
	 * Define the string representation of the DepPath. Suppose a DepPath records a path ARG1:prep:of:pobj:ARG2, if we
	 * call toString() without first setPath(), toString() will return ARG1:prep:of:pobj:ARG2. However, we might want
	 * toString() to return the transformed path. In that case, we can first setPath("ARG1:prep_of:ARG2"). toString() will
	 * return the transformed path thereafter.
	 * 
	 * @param path
	 *          User-defined string representation of the path
	 */
	...
	/**
	 * Create a new instance of DepPath from the current instance, attaching r to the tail of the new instance.
	 *
	 * @param r
	...
	public String toString() {
		if (path == null) {
			...
			for (int i = 0; i < relations.size(); i++) {
				...
				if (i == relations.size() - 1 || arg1 == null || arg2 == null
	...
	/**
	 * Linearize the DepPath, given the Jet Document, the SyntacticRelationSet, and the types of the entities at both
	 * ends.
	 * 
	 * @param doc
	 *          The Jet Document
	 * @param relations
	 *          Dependency relations extracted from the Jet Document
	 * @param type1
	 ...
	 */
	public String linearize(Document doc, SyntacticRelationSet relations, String type1, String type2, boolean stem) {
		...
		PriorityQueue<SyntacticRelation> nodes = new PriorityQueue<SyntacticRelation>(relations.size(),
		...
		// be printed out.
		SyntacticRelation startRel = new SyntacticRelation(-1, "", "", "NAMETAG", this.start, type1, "");
		nodes.add(startRel);
		...
		for (SyntacticRelation r : this.relations) {
			if (count == this.relations.size()) {
				...
				// path instead.
				SyntacticRelation fixedR = new SyntacticRelation(r.sourcePosn, r.sourceWord, r.sourcePos, r.type, r.targetPosn,
						"", r.targetPos);
				nodes.add(fixedR);
			...
			if (lexicalContent.length() > 0) {
				...
				SyntacticRelation prepRel = new SyntacticRelation(-1, "", "", "NODETYPE", offset, lexicalContent, "");
				nodes.add(prepRel);
		...
		SyntacticRelation endRel = new SyntacticRelation(-1, "", "", "NAMETAG", this.end, type2, "");
		nodes.add(endRel);
		...
		while (!nodes.isEmpty()) {
			...
			if (targetWord.equals("and") || targetWord.equals("or") || targetWord.equals(",")) {
				if (!(lastWord.equals(",") || lastWord.equals("or") || lastWord.equals("and") || lastWord.equals(""))) {
				...
				if (!targetWord.toLowerCase().trim().equals(lastWord) || targetWord.toUpperCase().equals(targetWord)) {
					Span targetSpan = new Span(node.targetPosn, node.targetPosn + 1);

					if (targetWord.toUpperCase().equals(targetWord) || arg1 == null || arg2 == null
							|| (!targetSpan.within(arg1) && !targetSpan.within(arg2))) {
	...
	private void addVerbDependents(SyntacticRelation r, SyntacticRelationSet relations, Document doc,
			PriorityQueue<SyntacticRelation> heap, TIntHashSet visitedOffsets) {
		if (posAt(r.targetPosn, doc).startsWith("V")) {
			...
			for (SyntacticRelation candidate : candidates) {
				...
				if (!candidate.virtual && (candidate.type.startsWith("dobj") || candidate.type.startsWith("nsubj")
	...
	private String wordOnPath(SyntacticRelation node, boolean stem) {
		...
		} else if (node.type.equals("dobj-1") && node.sourcePosn < node.targetPosn) {
			return node.targetWord.trim();
		} else if (stem) {
			return stemmer.getStem(node.targetWord.toLowerCase().trim(), node.targetPos).trim();
		} else {
			return node.targetWord.trim();
		}
	}
